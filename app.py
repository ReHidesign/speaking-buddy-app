...
st.set_page_config(page_title="Speaking Buddy v8", page_icon="🇬🇧")
...
if api_key:
    ...
    # --- FUNKCIÓK ---
    def speak_text(text):
        # Emojik eltávolítása a hangképzés előtt
    ...
    if not st.session_state.user_level:
        ...
        for i, m in enumerate(modes):
            if m_cols[i].button(m, use_container_width=True):
                # Emojik eltávolítása a belső mód azonosításhoz
        ...
        for idx, topic in enumerate(TOPICS):
            ...
            if t_cols[idx % 3].button(topic, use_container_width=True):
                st.session_state.chat_topic = clean_topic
                
                # Biztonságos kulcshasználat a hiba elkerülésére
                prompts = {
                    "Chat": f"Start a friendly discussion about {clean_topic}.",
                    "Game": f"We are in a roleplay about {clean_topic}. Start the scene as a character.",
                    "Picture": f"The topic is {clean_topic}. Describe a scene for me to visualize.",
                    "Test": f"Ask 3 challenging questions about {clean_topic}."
                }
                
                mode_key = st.session_state.current_mode
                ans = call_groq(prompts.get(mode_key, "Let's talk!"), "Be a helpful partner.")
        ...
        if user_msg:
            ...
            if "HELP" in user_msg.upper(): 
                instr = "First, correct any mistakes clearly. Then continue."
            answer = call_groq(user_msg, instr)
            st.session_state.messages.append({"role": "assistant", "content": answer})
            st.rerun()

    st.markdown("<br><hr><p style='text-align: center; color: grey; font-size: 12px;'>© 2026 Speaking Buddy App by ReHi | All Rights Reserved</p>", unsafe_allow_html=True)
