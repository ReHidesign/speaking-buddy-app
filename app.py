...
if api_key:
    ...
    def speak_text(text):
        # Narráció (zárójeles rész) eltávolítása a felolvasásból
    ...
    def call_groq(prompt, system_instruction):
        ...
        full_instr = (
            ...
            "Never use italics outside brackets. "
    ...
    if not st.session_state.intro_done:
        st.markdown("""
            <div class='buddy-container'>
                ...
                <p class='sub-title'>your interactive language speaking partner</p>
            </div>
            """, unsafe_allow_html=True)
        st.write("### Welcome!")
    ...
    # --- 2. LEVEL ---
        ...
        for i, l in enumerate(LEVELS.keys()):
            if cols[i%2].button(l, use_container_width=True):
                ...
                st.rerun()
        if st.button("🔍 Assess my level via chat", use_container_width=True):
            st.session_state.user_level = "Assessment"
            ans = call_groq("Start a level check. Question 1: What is your favorite way to spend a weekend?", "Level Assessor.")
    ...
    # --- 4. TOPIC ---
        ...
        for idx, topic in enumerate(TOPICS):
            ...
            if t_cols[idx%3].button(topic, use_container_width=True):
                ...
                prompt_map = {
                    "Picture": "I have a picture for you. Describe it in detail! *(Buddy shows the image)*",
                    "Debate": f"Let's debate {clean_t}. I'll start with a controversial statement. Do you agree?",
                    "Situation": f"We are in a real-life situation involving {clean_t}. *(Buddy sets the scene)*",
                    "Slang": f"Let's learn some cool idioms about {clean_t}!",
                    "Chat": f"Let's talk about {clean_t}."
                }
                ...
                ans = call_groq(prompt_map.get(st.session_state.current_mode), "Language Partner.")
                st.session_state.messages.append({"role": "assistant", "content": ans})
                st.rerun()
    ...
    # --- 5. CHAT ---
        ...
        if audio_data and 'bytes' in audio_data:
            # Itt lehetne Whisper API hívás, addig teszteljük szöveggel
            user_msg = "I heard your audio!" # Ide jön a Whisper kód
        elif text_input:
            user_msg = text_input

        if user_msg:
            st.session_state.messages.append({"role": "user", "content": user_msg})
            sys_m = f"You are in {st.session_state.current_mode} mode."
            if "HELP" in user_msg.upper(): sys_m = "Correct the student's grammar and give advice."
